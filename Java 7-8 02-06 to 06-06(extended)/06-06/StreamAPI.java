...
public class StreamAPI {
    public static void main(String[] args) {
        ...
        // print sq of given list after filtering even numbers
        list.stream().map(n -> n * n).filter(n -> n%2 == 0).forEach(n -> System.out.println(n));

        // collect as a list
        List<Integer> result = list.stream().map(n -> n * n).filter(n -> n%2 == 0).collect(Collectors.toList());
        System.out.println(result);

        List<Integer> marks = Arrays.asList(80, 50, 95, 75, 60, 88);

        int total = marks.stream().reduce((n1, n2) -> n1 + n2).get();
        ...
        List<Integer> finalList = list1.stream().flatMap(li -> li.stream()).map(n -> n*2).collect(Collectors.toList());
