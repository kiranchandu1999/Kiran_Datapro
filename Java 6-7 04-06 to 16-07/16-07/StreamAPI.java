...
public class StreamAPI {
    public static void main(String[] args) {
        ...
        list.stream().map(n -> n*n).filter(k -> k%2 == 0).forEach(n -> System.out.println(n));

        List<Integer> res = list.stream().map(n -> n*n).filter(k -> k%2 == 0).collect(Collectors.toList());
        ...
        System.out.println(marks.stream().anyMatch(m -> m > 90)); // true

        System.out.println(marks.stream().allMatch(m -> m > 90)); // false
